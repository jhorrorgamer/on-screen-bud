...
class DesktopBuddyApp:
    ...
    def draw_buddy(self, foot_offset: int) -> None:
        ...
        if not self.transparent_supported:
            self.canvas.create_rectangle(0, 0, SIZE, SIZE, fill=FALLBACK_BG, outline=FALLBACK_BG)
    ...
    def move_window(self) -> None:
        ...
        if random.random() < 0.02:
            self.vx = max(-3, min(3, self.vx + random.choice([-1, 1])))
            self.vy = max(-3, min(3, self.vy + random.choice([-1, 1])))
            if self.vx == 0:
                self.vx = 1
            if self.vy == 0:
                self.vy = -1
